...
import certifi
import asyncio
from telebot.types import ReplyKeyboararkup, KeyboardButton
from threading import Thread

loop = asyncio.get_event_loop()
TOKEN = '8003599609:AAGFDAmeFxBrOjjDuQ2WNOg0szzAvbL_7rM'
MONGO_URI = 'mongodb+srv://ihatemosquitos9:JvOK4gNs0SH5SVw9@cluster0.1pd5kt5.mongodb.net/?appName=Cluster0'
CHANNEL_ID = 7050279728
...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*🚫 YOU ARE NOT APPROVED 🚫\n\nOops! It seems like you don't have permission to use the Attack command. To gain access and unleash the power of attacks, you can:\n👉 Contact an Admin or the Owner for approval @KILLERDDOS*", parse_mode='Markdown')
@bot.message_handler(commands=['approve', 'disapprove'])
def approve_or_disapprove_user(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    is_admin = is_user_ain(user_id, CHANNEL_ID)
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "*🔆 WELCOME TO VIP KILLER DDOS BOT 🔆*", reply_markup=markup, parse_mode='Markdown')
...
def handle_message(message):
    ...
    if message.text == "Buy Access! 💰":
        bot.reply_to(message, "* 𝗩𝗜𝗣 𝗗𝗗𝗢𝗦 𝗣𝗥𝗜𝗖𝗘\n\n[𝗣𝗿𝗲𝗺𝗶𝘂𝗺]\n> DAY - 200 INR\n> WEEK - 700 INR\n\n[𝗣𝗹𝗮𝘁𝗶𝗻𝘂𝗺]\n> MONTH - 1600 INR\n\nDM TO BUY @KILLERDDOS*", parse_mode='Markdown')
